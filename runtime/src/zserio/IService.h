 ...
 * getData() is used.
 */
template <typename ZSERIO_OBJECT, typename ALLOC = std::allocator<std::uint8_t>>
...
public:
     ...
     */
    explicit BasicRawServiceDataHolder(const Vector<std::uint8_t, ALLOC>& rawData) :
            m_data(rawData)
    {}
     ...
     */
    explicit BasicRawServiceDataHolder(Vector<std::uint8_t, ALLOC>&& rawData) :
...
private:
    Vector<std::uint8_t, ALLOC> m_data;
};
...
public:
     ...
     */
    explicit BasicRawServiceDataView(zserio::Span<const std::uint8_t> rawData) :
            m_data(rawData)
    {}
...
public:
    ...
    virtual IBasicServiceDataPtr<ALLOC> callMethod(
            std::string_view methodName, Span<const std::uint8_t> requestData, void* context) = 0;
};
...
public:
     ...
     * \throw ServiceException if the call fails.
     */
    virtual Vector<std::uint8_t, ALLOC> callMethod(
            std::string_view methodName, const IBasicServiceData<ALLOC>& requestData, void* context) = 0;
};

/** Typedef to service interface provided for convenience - using default std::allocator<std::uint8_t>. */
/** \{ */
